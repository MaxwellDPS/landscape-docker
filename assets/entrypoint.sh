...
appInit () {
  ...
  sed -i 's,{{RMQ_VHOST}},'"${RMQ_VHOST}"',g' /etc/landscape/service.conf
    ...
    sed -i 's,{{SERVER_NAME}},'"${SERVER_NAME}"',g' /etc/apache2/sites-available/landscape.conf
...
appSchema () {
  ...
  # ${psqlconn} -c \"ALTER SYSTEM SET max_connections=400;\"
  # ${psqlconn} -c \"ALTER SYSTEM SET max_prepared_transactions=400;\"
